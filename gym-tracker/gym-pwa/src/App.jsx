...
// Front-view body silhouette with highlighted muscle groups
function MuscleIcon({ groups = [], size = 36 }) {
  // groups: array of "quads","hams","glutes","calves","abs","obliques","chest","lats","traps","delts","biceps","triceps","forearms"
  const h = size * 1.4;
  ...
  return (
    <svg viewBox="0 0 44 62" width={size} height={h} style={{ display: "block", flexShrink: 0 }}>
      ...
      {/* Erectors (lower back, shown faintly through torso) */}
...
const MUSCLE_MAP = {
  nr1:      ["quads", "glutes", "hams", "calves"],      // Leg Press
  nr2a:     ["quads"],                                    // Leg Extension
  nr2b:     ["hams"],                                     // Leg Curl
  nr3:      ["glutes", "hip"],                            // Hip Abduction
  nr4:      ["obliques", "abs"],                          // Lateral Flexion
  nr5:      ["obliques", "abs"],                          // Torso Rotation
  nr6:      ["abs"],                                      // Ab Crunch
  nr7:      ["erectors", "glutes"],                       // Back Extension
  nr8:      ["traps", "delts"],                           // Reverse Fly
  nr9:      ["triceps", "chest", "delts"],                // Tricep Dips
  nr10:     ["lats", "traps", "biceps"],                  // Rowing
  nr11:     ["lats", "biceps"],                           // Lat Pulldown
  nr12:     ["erectors", "abs"],                          // Back Trainer
  sz_rot:   ["obliques", "abs"],                          // Cable Torso Rotation
  trx:      ["lats", "biceps", "abs"],                    // TRX Row
  sz_huft:  ["hip", "glutes", "hams"],                    // Cable Hip Ext/Flex
  brustpr:  ["chest", "delts", "triceps"],                // Cable Chest Press
  // BW
...
function ExerciseDetail({ exercise, entries, rom, isBW, onAdd, onEdit, onDelete, onBack, onROM }) {
  ...
  return (
    <div>
      ...
      {/* Larger muscle diagram in detail view */}
